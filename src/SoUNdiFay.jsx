 proyecto Vite, subirlo a GitHub, e importarlo en Vercel. Lo más delicado es que la URL final de Vercel coincida exactamente con el REDIRECT_URI que está hardcodeado en el código (https://soundify-4fjj.vercel.app/callback), y que esa misma URL esté registrada en el Dashboard de Spotify como Redirect URI autorizada.

Si en cambio ya tenías este proyecto desplegado en Vercel antes y solo querés subir el arreglo del buscador que acabamos de hacer, es mucho más simple: copiás el código actualizado del artefacto a tu archivo local (probablemente src/App.jsx), hacés git add, git commit, git push, y Vercel redespliega solo.

¿Cuál de los dos casos es el tuyo? Si es la segunda opción (ya tenías el proyecto andando), te puedo ayudar más puntual con ese paso.

ya estaba em vercel solo tengo que subir los cambios

10:13 a. m.
Entonces es simple. Tenés que copiar el código actualizado del artefacto (el de arriba, con el buscador arreglado) a tu archivo local, y subirlo.

Pasos:

Abrí tu proyecto local donde tenés el repo de SoUNdiFay.
Copiá todo el contenido del artefacto de código (el que dice SoUNdiFay.jsx — Con Stats arriba) y pegalo reemplazando el contenido completo de tu archivo App.jsx (o como se llame el archivo donde tenés el componente).
Desde la terminal, en la carpeta del proyecto:
bash
git add .
git commit -m "Fix: el buscador perdía el foco al escribir"
git push
Vercel va a detectar el push automáticamente y va a redesplegar la app sola. No hace falta tocar nada en el Dashboard de Spotify ni cambiar ninguna configuración, porque el REDIRECT_URI y el resto de la lógica de auth no cambiaron, solo se arregló el componente del buscador.

En un minuto o dos deberías ver el deploy nuevo reflejado en https://soundify-4fjj.vercel.app. Si querés verificar que terminó, podés mirar la pestaña "Deployments" en el dashboard de Vercel de tu proyecto.

screenshot.png
10:17 a. m.
El push se hizo bien (veo el commit "Fix: el buscador perdía el foco al escribir"), pero el build falló con "Command npm run build exited with 1". El log que se ve corta justo antes de mostrar el error real, que tiene que estar más abajo en esas 13 líneas.

Necesito ver el error específico para decirte qué falla. ¿Podés hacer scroll hacia abajo en esos "Deploy Logs" y pasarme las líneas que vienen después de "31 packages are looking for funding"? Ahí debería estar el mensaje de error concreto (algo tipo "Failed to compile" con el archivo y línea exacta).

Mientras tanto, te tiro las causas más comunes de que esto falle justo después de pegar el código del artefacto:

Una posibilidad es que al copiar el código se haya pegado mal algún carácter especial (las comillas tipográficas „ " en vez de comillas normales " son un clásico al copiar desde un navegador).

Otra posibilidad es que falte alguna dependencia. Este componente no usa librerías externas raras (solo React), así que no debería ser eso, pero si tu package.json tenía algo distinto antes podría chocar.

También puede ser un problema de sintaxis JSX si algo quedó cortado al copiar y pegar (una llave o paréntesis de menos).

Pasame el resto del log de error y lo resolvemos directo.
...
const REDIRECT_URI = "https://soundify-4fjj.vercel.app/callback";
const SCOPES = [
...
const IC = {
  ...
  stats:     <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="22" height="22"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>,
  ...
  music:     <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="22" height="22"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>,
};
...
// ── StatCard ──────────────────────────────────────────────────────
function StatCard({ emoji, label, value, sub, color="#818cf8" }) {
  return (
    <div style={{ background:"#ffffff08", borderRadius:16, padding:"16px", border:"1px solid #ffffff10", display:"flex", flexDirection:"column", gap:6 }}>
...
// ── GenreBar ──────────────────────────────────────────────────────
function GenreBar({ genre, pct, color }) {
  return (
    <div style={{ marginBottom:10 }}>
      ...
      <div style={{ height:6, background:"#ffffff14", borderRadius:4, overflow:"hidden" }}>
...
export default function SoUNdiFay() {
  ...
  // Stats
  const [statsRange,    setStatsRange]    = useState("short_term");
  ...
  useEffect(()=>{
    ...
    Promise.all([
      api(`/me/top/tracks?limit=50&time_range=${statsRange}`,token).then(d=>setStatsTracks(d.items||[])),
      api(`/me/top/artists?limit=50&time_range=${statsRange}`,token).then(d=>setStatsArtists(d.items||[])),
    ]).finally(()=>setStatsLoading(false));
  },[token,tab,statsRange]);
      ...
      <>
        ...
        <div style={{ display:"flex", flexDirection:wide?"row":"column", alignItems:wide?"flex-start":"center", gap:wide?32:10, padding:wide?"24px 32px":"8px 20px 16px", textAlign:wide?"left":"center" }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ color:C.muted, fontSize:15, margin:"6px 0" }}>{albumDetail.artists?.map(a=>a.name).join(", ")}</div>
        ...
        <div style={{ display:wide?"grid":"flex", gridTemplateColumns:wide?"1fr 1fr":"unset", flexDirection:wide?"unset":"column" }}>
          {tracks.map((t,i)=>(
            <div key={t.id||i} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 20px", cursor:"pointer" }}
    ...
    return(
      <>
        ...
        <div style={{ display:"flex", flexDirection:wide?"row":"column", alignItems:wide?"flex-start":"center", gap:wide?32:8, padding:wide?"24px 32px":"8px 20px 16px", textAlign:wide?"left":"center" }}>
          <Thumb src={playlistDetail.images?.[0]?.url} alt={playlistDetail.name} style={{ width:wide?200:170, height:wide?200:170, borderRadius:14, objectFit:"cover", flexShrink:0, boxShadow:"0 10px 40px #00000077" }} emoji="🎵"/>
          <div style={{ flex:1 }}>
  ...
  function StatsTab() {
    // Géneros
    const genreMap = {};
    statsArtists.forEach(a=>{
      (a.genres||[]).forEach(g=>{ genreMap[g]=(genreMap[g]||0)+1; });
    });
    const totalGenre = Object.values(genreMap).reduce((a,b)=>a+b,0)||1;
    const topGenres = Object.entries(genreMap)
      .sort((a,b)=>b[1]-a[1]).slice(0,8)
      .map(([g,c])=>({ g, pct:Math.round((c/totalGenre)*100) }));

    // Décadas
    const decadeMap = {};
    statsTracks.forEach(t=>{
      const y = t.album?.release_date?.slice(0,4);
      if(y){ const d=`${Math.floor(+y/10)*10}s`; decadeMap[d]=(decadeMap[d]||0)+1; }
    });
    const topDecades = Object.entries(decadeMap).sort((a,b)=>b[1]-a[1]).slice(0,5);

    // Popularidad promedio
    const avgPop = statsTracks.length
      ? Math.round(statsTracks.reduce((a,t)=>a+(t.popularity||0),0)/statsTracks.length)
      : 0;

    // Duración promedio
    const avgMs = statsTracks.length
      ? Math.round(statsTracks.reduce((a,t)=>a+(t.duration_ms||0),0)/statsTracks.length)
      : 0;

    // Artista más repetido en top tracks
    const artistCount = {};
    statsTracks.forEach(t=>{ t.artists?.forEach(a=>{ artistCount[a.name]=(artistCount[a.name]||0)+1; }); });
    const topArtistName = Object.entries(artistCount).sort((a,b)=>b[1]-a[1])[0];

    // Álbumes únicos
    const uniqueAlbums = new Set(statsTracks.map(t=>t.album?.id)).size;

    const GENRE_COLORS = ["#818cf8","#a78bfa","#34d399","#f472b6","#fb923c","#38bdf8","#facc15","#f87171"];

    if(statsLoading) return <div style={{ textAlign:"center", padding:48, color:C.muted }}>Calculando tus stats…</div>;

    return(
      <>
        <SecTitle>Tus estadísticas</SecTitle>
        <TimeRangeSelector value={statsRange} onChange={setStatsRange}/>

        {/* Tarjetas resumen */}
        <div style={{ display:"grid", gridTemplateColumns:wide?"repeat(3,1fr)":"repeat(2,1fr)", gap:10, padding:`0 ${wide?24:16}px 16px` }}>
          <StatCard emoji="🎵" label="Canciones analizadas" value={statsTracks.length} color={C.accent}/>
          <StatCard emoji="🎤" label="Artistas únicos" value={statsArtists.length} color="#34d399"/>
          <StatCard emoji="💿" label="Álbumes distintos" value={uniqueAlbums} color="#f472b6"/>
          <StatCard emoji="⭐" label="Popularidad media" value={`${avgPop}/100`} color="#facc15"/>
          <StatCard emoji="⏱️" label="Duración media" value={fmtMs(avgMs)} color="#fb923c"/>
          {topArtistName&&<StatCard emoji="👑" label="Artista favorito" value={topArtistName[0]} sub={`${topArtistName[1]} canciones en tu top`} color="#f87171"/>}
        </div>

        {/* Top canción */}
        {statsTracks[0]&&(
          <>
            <SecTitle sub="La canción que más escuchas">🏆 Tu #1</SecTitle>
            <div style={{ margin:`0 ${wide?24:16}px 16px`, background:"linear-gradient(135deg,#312e8180,#4c1d9580)", borderRadius:16, padding:16, display:"flex", alignItems:"center", gap:14 }}>
              <Thumb src={statsTracks[0].album?.images?.[0]?.url} alt={statsTracks[0].name}
                style={{ width:70, height:70, borderRadius:10, objectFit:"cover", flexShrink:0 }}/>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontWeight:800, fontSize:16 }}>{statsTracks[0].name}</div>
                <div style={{ color:C.muted, fontSize:13, marginTop:2 }}>{statsTracks[0].artists?.map(a=>a.name).join(", ")}</div>
                <div style={{ color:C.subtle, fontSize:11, marginTop:4 }}>Popularidad: {statsTracks[0].popularity}/100</div>
              </div>
              <PlayBtn previewUrl={statsTracks[0].preview_url} playingUrl={playingUrl} playing={playing} onToggle={()=>playTrack(statsTracks[0],statsTracks)} size={36}/>
            </div>
          </>
        )}
        ...
        {topGenres.length>0&&(
          <>
            <SecTitle sub="Basado en tus artistas top">🎸 Tus géneros</SecTitle>
            <div style={{ padding:`0 ${wide?24:16}px 16px` }}>
              {topGenres.map(({g,pct},i)=>(
                <GenreBar key={g} genre={g} pct={pct} color={GENRE_COLORS[i%GENRE_COLORS.length]}/>
              ))}
            </div>
          </>
        )}
        ...
        {topDecades.length>0&&(
          <>
            <SecTitle sub="De qué época es tu música">📅 Décadas favoritas</SecTitle>
            <div style={{ display:"flex", gap:10, padding:`0 ${wide?24:16}px 16px`, flexWrap:"wrap" }}>
              {topDecades.map(([d,c],i)=>(
                <div key={d} style={{ background:GENRE_COLORS[i]+"22", border:`1px solid ${GENRE_COLORS[i]}44`, borderRadius:12, padding:"10px 16px", textAlign:"center" }}>
                  <div style={{ fontSize:18, fontWeight:800, color:GENRE_COLORS[i] }}>{d}</div>
                  <div style={{ fontSize:11, color:C.muted, marginTop:2 }}>{c} canciones</div>
                </div>
              ))}
            </div>
          </>
        ...
        {/* Top artistas con popularidad */}
        {statsArtists.length>0&&(
          <>
            <SecTitle sub="Ordenados por tus reproducciones">🎤 Top artistas</SecTitle>
            <div style={{ display:wide?"grid":"flex", gridTemplateColumns:wide?"1fr 1fr":"unset", flexDirection:wide?"unset":"column" }}>
              {statsArtists.slice(0,10).map((a,i)=>(
                <div key={a.id||i} style={{ display:"flex", alignItems:"center", gap:12, padding:`8px ${wide?20:16}px` }}>
                  <div style={{ width:26, textAlign:"center", flexShrink:0 }}>
                    {i<3?<span style={{ fontSize:18 }}>{["🥇","🥈","🥉"][i]}</span>
                        :<span style={{ color:C.subtle, fontSize:13, fontWeight:600 }}>{i+1}</span>}
                  </div>
                  <Thumb src={a.images?.[0]?.url} alt={a.name} style={{ width:46, height:46, borderRadius:"50%", objectFit:"cover", flexShrink:0 }} emoji="🎤"/>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontWeight:600, fontSize:13, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{a.name}</div>
                    <div style={{ color:C.muted, fontSize:11 }}>{a.genres?.[0]||"Artista"} · ⭐ {a.popularity}/100</div>
                  </div>
        ...
        {/* Top canciones con duración */}
        {statsTracks.length>0&&(
          <>
            <SecTitle sub="Las que más suenan en tu historial">🎵 Top canciones</SecTitle>
            <div style={{ display:wide?"grid":"flex", gridTemplateColumns:wide?"1fr 1fr":"unset", flexDirection:wide?"unset":"column" }}>
              {statsTracks.slice(0,20).map((t,i)=>(
                <div key={t.id||i} style={{ display:"flex", alignItems:"center", gap:10, padding:`8px ${wide?20:16}px`, cursor:"pointer" }}
                  onClick={()=>playTrack(t,statsTracks)}>
                  <div style={{ width:26, textAlign:"center", flexShrink:0 }}>
                    {i<3?<span style={{ fontSize:18 }}>{["🥇","🥈","🥉"][i]}</span>
                        :<span style={{ color:C.subtle, fontSize:13, fontWeight:600 }}>{i+1}</span>}
                  </div>
                  ...
                  <PlayBtn previewUrl={t.preview_url} playingUrl={playingUrl} playing={playing} onToggle={()=>playTrack(t,statsTracks)} size={28}/>
                </div>
              ))}
            </div>
          </>
        ...
        {statsTracks.length>0&&(()=>{
          ...
          statsTracks.forEach(t=>{
            ...
            if(!albumMap[t.album.id]) albumMap[t.album.id]={ album:t.album, count:0 };
            albumMap[t.album.id].count++;
          });
          const topAlbums = Object.values(albumMap).sort((a,b)=>b.count-a.count).slice(0,8);
          return(
            <>
              <SecTitle sub="Los álbumes con más canciones en tu top">💿 Álbumes más escuchados</SecTitle>
              <HScroll>
                {topAlbums.map(({album,count})=>(
                  <div key={album.id} onClick={()=>openAlbum(album.id)} style={{ width:wide?130:110, flexShrink:0, cursor:"pointer" }}>
                    <Thumb src={album.images?.[0]?.url} alt={album.name} style={{ width:wide?130:110, height:wide?130:110, borderRadius:10, objectFit:"cover" }} emoji="💿"/>
                    <div style={{ fontSize:11, fontWeight:700, marginTop:6, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{album.name}</div>
                    <div style={{ fontSize:10, color:C.muted }}>{count} canción{count>1?"es":""} en tu top</div>
                  </div>
                ))}
              </HScroll>
            </>
  ...
  function ArtistsTab() {
    return(
      <>
        ...
        <div style={{ display:wide?"grid":"flex", gridTemplateColumns:wide?"1fr 1fr":"unset", flexDirection:wide?"unset":"column" }}>
          {topArtists.map((a,i)=>(
            <ListItem key={a.id||i} img={a?.images?.[0]?.url} imgRound title={a?.name} sub={a?.genres?.[0]||"Artista"} emoji="🎤">
              <SpotifyLink url={a?.external_urls?.spotify} size={26}/>
            </ListItem>
          ))}
        </div>
      ...
      <>
        ...
        {srArtists.length>0&&<>
          ...
          <HScroll>{srArtists.map(a=>(
            <div key={a.id} style={{ width:90, flexShrink:0, textAlign:"center" }}>
